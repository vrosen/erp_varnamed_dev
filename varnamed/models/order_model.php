...
Class Order_model extends CI_Model {
    ...
    /**
     * check avalable of product in whare house
     * reduce whare house quantity
     * set backorder flag in in order items and order
     * create backorder records if needed
     * for more clearance operation are removed i separate methods
     * 
     * @param type $data arrtay of product
     * @param type $customer data about customer
     */
    function insert_resevations($data, $customer) {
        // TO DO reform data remove complex add parts
        // utility data complex part 
        // mark in utility data checked object
        // finally add check for complex part
        // add additionally field complex in bacorders tatable that mightg 
        // to be ind and related indexes
        // 
        // 
echo "<pre>";
print_r($data);
...
print_r($product);
            ...
            // or alternativelly check type
            $complexType = $this->getProductComplexType(
                    $product['shop_id'], 
 ...
 print_r($parts);
                ...
                // will use to determine if compose product is in backorder
                $readyFlag = true;
                // cycle through parts
                foreach($parts as $subProduct)
                {
                    $subProductForReservation = $productForReservation;
                    while(true)
                    {
                        $this->db->trans_begin();
                        $oldest = $this->getOldestWarehouse(
                            $subProduct['shop_id'],
                            $subProduct['code']
                        );
                        // continue here
                        print_r($oldest);
                        if (!$oldest || empty($oldest->id)) {
                            // set flag false so finally mark compose object 
                            // in order
                            $readyFlag = false; 
                            // save in backorder
                            $this->setBackOrder(
                            ...
                            if ($this->db->trans_status() === FALSE) {
                                ...
                                break; // GO to next sub product
                            }
                        }
                        else{
                            if ($oldest->current_quantity - $oldest->ordered_quantity >= $subProductForReservation) {
                                 // change warehouse_order_products  ordered_quantity
                                // no back orders
                                $this->updateWarehouseorder(
                                        $subProductForReservation,
                                        $oldest->id
                                );
                                // set order_item backorder = 0
                                $this->setBackOrderItems(
                                        $subProduct['shop_id'],
                                        $product['order_id'], 
                                        $subProduct['code'], 
                                        0
                                ...
                                $this->closeWarehouseorder($oldest);
                                //
            ...
            {
                // for more clear move sql parts out side 
                while (true) {
    ...
    // utility methods for $this->insert_resevations ------------------------
    /**
    ...
    /**
     * set backorder flag in order item
     * TO DO if multiple items ??
     * @param type $shop_id shop
     * @param type $order_id order
     * @param type $code  product code
     * @param type $backOrder  0,1 
     */
    public function setBackOrderItems($shop_id,$order_id, $code, $backOrder=0)
    {
        $code = $this->clearTerm($code);
        $sql = "UPDATE order_items SET BACKORDER = $backOrder "
    ...
    /**
     * Get oldes free free warehouse order
     * @param type $shop_id store
     * @param type $code    product code
     * @return type oldest free warehouse order object
     */
    public function getOldestWarehouse($shop_id,$code)
    {
        $code = $this->clearTerm($code);
    ...
    /**
     * Insert backorder records
     * @param type $order_id Order id
     * @param type $shop_id  Shop id
     * @param type $code Product code
     * @param type $productForReservation quontity missing
     * @param type $order_number Order nuber
     * @param type $company Customer company name
     * @param type $field_service  filed_service Id
     */
    public function setBackOrder(
                $order_id,
                $shop_id, 
                $code,
                $productForReservation, 
    ...
    {
        $code = $this->clearTerm($code);
        $sql = "INSERT backorder SET order_id='" . $order_id . "', "
            . " shop_id ='" . $shop_id . "', "
            . " product_code='" . $code . "', "
            . " backorder_quantity='" . $productForReservation . "', "
            . " current_stock=0, "
            . " order_number ='" . $order_number . "', "
            . " order_date ='" . $reservation_date . "', "
            . " backorder_date ='" . $reservation_date . "', "
            . " customer ='" . addslashes($company) . "', "
            . " agent_id ='" . $field_service . "', "    
            . " agent_name ='" . $this->session->userdata('ba_username') . "' ";
        if($complexCode)
            $sql .= ", complex_code='".$complexCode."' ";
        $query2 = $this->db->query($sql);
    }
    /**
     * fix warehouse worder withused quontity
     * @param type $productForReservation used quontity
     * @param type $id order Id
     */
    public function updateWarehouseorder($productForReservation,$id)
    {
    ...
    /**
     * mark wharehouse order as closed
     * @param type $oldest waterhowse order object
     */
    public function closeWarehouseorder($oldest)
    {
    ...
    /* end utility methods for $this->insert_resevations                      */
    /* ---------------------------------------------------------------------- */
