...
Class Tax_model extends CI_Model
{
	...
	function get_zone_tax_rate()
	{
		...
		$rate	= $this->db->where('id', $this->address['zone'])->get('country_zones')->row();
		if($rate)
		{
			$rate	= $rate->tax/100;
		}
	...
	function get_area_tax_rate()
	{
		...
		$rate	= $this->db->where(array('code'=>$this->address['zip'], 'zone'=>$this->address['zone']))->get('country_zone_areas')->row();
		if($rate)
		{
			$rate	= $rate->tax/100;
		}
