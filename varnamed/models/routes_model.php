...
class Routes_model extends CI_Model {
	...
	function save($route){
		...
		function save_product_route($data) {

			$this->db->where('id', $data['route_id']);
			$update = array('product_id'=>$data['product_id'],'route'=>$data['route']);
			
			$this->db->update('routes', $update);
		...
		$this->db->where('group_id',$data['group_id']);
		$result = $this->db->get('category_groups');
		...
		function save_product_relation($data) {
		
		$this->db->where('product_id',$data['product_id']);
		$result = $this->db->get('group_products');
		
		if ($result->num_rows() > 0) {
					
			$this->db->where('product_id', $data['product_id']);
			$this->db->update('group_products', $data);
