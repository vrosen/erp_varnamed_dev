...
<?php echo form_open($this->config->item('admin_folder').'/overview/sales1');?>
...
<table class="table table-striped table-condensed" style="width:1800px;">
    <tbody style="display:block; height: 600px; overflow-y: auto;" >
        <?php
            ...
            $s['total'] = 0;
            $qclbase = $this->config->item('admin_folder')."/overview/sales1/$select_type_overview/";
        ?>
        <?php foreach($res as $k => $v): ?>
            <tr>
                <td style='width: 400px'>
                    <?php 
                        $q = $this->config->item('admin_folder')."/overview/sales1/$select_type_overview/";
                        if(empty($k) ) $k = "Na: ";
