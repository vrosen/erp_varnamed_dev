					...
					<table class="table table-striped table-hover" style="border: 1px solid #ddd;">
						<thead>
							<tr>
								...
								<?php endif; ?>
											<th style="font-size:12px;"><?php echo sort_url('order', 'order_number', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px;"><?php echo sort_url('company', 'company', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px;"><?php echo sort_url('entered_by','entered_by', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px; width: 15px; ">Pick<?php //echo sort_url('entered_by','entered_by', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px; width: 15px;">Mon<?php //echo sort_url('entered_by','entered_by', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px; width: 15px;">Lab<?php //echo sort_url('entered_by','entered_by', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px;"><?php echo sort_url('ordered_on','ordered_on', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px;"><?php echo sort_url('processed_on','processed_on', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px;"><?php echo sort_url('dispatch_method','shipping_method', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<th style="font-size:12px;"><?php echo sort_url('versendung','total', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
											<?php 
												if($this->data_shop == 3){
							...
							<tbody>
							<?php echo (count($new_shipments) < 1)?'<tr><td style="text-align:center;" colspan="8">'.lang('no_orders') .'</td></tr>':''?>
								<?php if(!empty($new_shipments)):?>
							<?php foreach($new_shipments as $new_shipment): ?>
							<tr>
								<td><input name="order[]" type="checkbox" value="<?php echo $new_shipment->id; ?>" class="gc_check"/></td>
								<td style="font-size:12px;"><?php echo $new_shipment->order_number; ?></td>
								<td style="font-size:12px;"><?php echo $new_shipment->company; ?></td>
								<td style="font-size:12px;"><?php echo $new_shipment->entered_by; ?></td>
								<td style="font-size:12px;"><?php echo $new_shipment->picking_agent; ?></td>
								<td style="font-size:12px;"><?php echo $new_shipment->monitored_by; ?></td>
								<td style="font-size:12px;"><?php echo $new_shipment->label_number; ?></td>
								<td style="font-size:12px;"><?php echo date('m/d/y h:i a', strtotime($new_shipment->ordered_on)); ?></td>
										
										<td style="font-size:12px;"><?php echo $new_shipment->shipped_on; ?></td>
										<td style="font-size:12px;"><?php echo $new_shipment->shipping_method; ?></td>
									...
									<td style="text-align: center; width: 50px;">
										<a class="glyphicon glyphicon-edit" style="display: inline-block; font-size: 16px;" href="<?php echo site_url($this->config->item('admin_folder').'/orders/view/'.$new_shipment->id);?>"><?php //echo lang('form_view')?></a>
									</td>
									<td style="text-align: center; width: 50px;">	
										<a class="glyphicon glyphicon-th-list" style="display: inline-block; font-size: 16px;" href="<?php echo site_url($this->config->item('admin_folder').'/orders/order_details/'.$new_shipment->id);?>"><?php //echo lang('form_view')?></a>
									</td>
