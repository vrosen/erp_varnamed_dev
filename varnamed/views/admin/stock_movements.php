<?php include('header.php');
    ...
    function sort_url($lang, $by, $sort, $sorder, $code, $admin_folder){
            ...
            $return = site_url($admin_folder.'/stock/movements/'.$by.'/'.$sort.'/'.$code);
			...
			<div class="span8">
				<?php echo form_open($this->config->item('admin_folder').'/products/index', 'class="form-inline" style="float:right"');?>
					<fieldset>
						...
						<a class="btn" href="<?php echo site_url($this->config->item('admin_folder').'/stock/movements');?>">Reset</a>
					</fieldset>
				</form>
			</div>
		</div>
...
<?php echo form_open($this->config->item('admin_folder').'/products/bulk_save', array('id'=>'bulk_form'));?>
	...
	<table class="table table-striped">
		<thead>
			<tr>
				...
				<th><?php echo sort_url('date', 'name', $order_by, $sort_order, $code, $this->config->item('admin_folder'));?></th>
                                <th><?php echo sort_url('warehouse_place', 'warehouse', $order_by, $sort_order, $code, $this->config->item('admin_folder'));?></th>
	...
	<?php foreach ($products as $product):?>
                            <tr>
				<td><?php echo form_input(array('name'=>'product['.$product->id.'][code]','value'=>form_decode($product->code), 'class'=>'span2'));?></td>
                                <td><?php echo form_input(array('name'=>'product['.$product->id.'][code]','value'=>form_decode($product->productnr), 'class'=>'span1'));?></td>
                                <td><?php echo form_input(array('name'=>'date','class'=>'span1'));?></td>
                                ...
                                <td>
                                    ...
                                    $options = array(
                                        ...
                                        echo form_dropdown('product['.$product->id.'][enabled]', $options, set_value('enabled',$product->enabled), 'class="span3"');
                                    ?>
				</td>
                                <td></td>
                                <td></td>
