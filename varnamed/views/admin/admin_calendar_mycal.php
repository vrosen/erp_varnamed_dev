...
?>
	...
	<table cellspacing="0">
				...
				for($j=0; $j<7;$j++)
				{
					...
					if($day>0 && $day<=$total_days_of_current_month)
					{
						...
						if(array_key_exists($day,$events))
						{
							...
							echo  '<li>';
							echo anchor("/admin/edit/$single->id",'<span class="title">'.$single->eventTitle.'(by '.$single->user.')</span><span class="desc">'.$single->eventContent.'</span>');
							echo '</li>'; 
		...
		<tfoot>		
			<th>
			<?php echo anchor('admin/mycal/'.$user_id."/".$previous_year,'&laquo;&laquo;', array('title'=>$previous_year_text));?>
			</th>
			<th>
			<?php echo anchor('admin/mycal/'.$user_id."/".$previous_month,'&laquo;', array('title'=>$previous_month_text));?>
			</th>
			<th>&nbsp;</th>
			<th>&nbsp;</th>
			<th>&nbsp;</th>
			<th>
			<?php echo anchor('admin/mycal/'.$user_id."/".$next_month,'&raquo;', array('title'=>$next_month_text));?>
			</th>
			<th>
			<?php echo anchor('admin/mycal/'.$user_id."/".$next_year,'&raquo;&raquo;', array('title'=>$next_year_text));?>
...
foreach ($members->result_array() as $member){
	echo "<p>";
	echo anchor('admin/mycal/'.$member['id'],$member['username'].'\'s Calendar' );
	echo "</p>";
